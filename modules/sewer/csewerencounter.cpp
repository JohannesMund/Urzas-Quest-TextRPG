...
void CSewerEncounter::execute(const std::string_view& moduleName)
{
    ...
    Console::printLn(
        tr("As you enter {}, the whole town is in an uproar. People running around, chattering, some are "
    ...
    Console::printLn(tr(
        ...
        CC::ccReset(),
        townName));
    Console::printLn(tr("Do you accept your new quest?"));
    Console::br();

    if (CMenu::executeAcceptRejectMenu() == CMenu::reject())
    {
        Console::printLn(tr("You decide not to. You give the townsfolk a lecture about \"You-Problems vs. Me-Problems\", "
                            "and reject their quest."));
        Console::printLn(
            tr("At least, they understand. The shop is still open for you and the tavern sells you some "
               "beer. The Barkeeper probably spit into it, and you should definitely check the equipment you buy today."));
        return;
    }

    Console::printLn(
        tr("Of course you will! This is what you are here for. The townspeople show you the entry to the sewer and you "
