#pragma once

#include "moduleinfo.h"

#include <string>
#include <vector>

namespace TagNames
{
namespace WonderLamp
{
constexpr std::string_view wonderlamp = "Wonderlamp";
constexpr std::string_view findWonderlamp = "FindWonderlamp";
constexpr std::string_view djinn = "Djinn";
constexpr std::string_view genie = "Genie";
constexpr std::string_view jeannie = "Jeannie";
constexpr std::string_view djinnCompanion = "DjinnCompanion";
constexpr std::string_view gem = "Gem";
constexpr std::string_view gemInteractiom = "GemInteraction";
constexpr std::string_view genieTalkInteraction = "GenieTalkInteraction";
constexpr std::string_view jeannieTalkInteraction = "JeannieTalkInteraction";

} // namespace WonderLamp
} // namespace TagNames

class CRoom;
class CItem;

namespace WonderLamp
{
Module::ModuleInfo moduleInfo();
std::string moduleName();

std::string djinn();

std::string colorizeJeannie(const std::string_view& s);
std::string jeannie();

std::string colorizeGenie(const std::string_view& s);
std::string genie();

std::string wonderlamp();
std::string wonderLampEncounter();

std::string getRandomGem();

} // namespace WonderLamp
