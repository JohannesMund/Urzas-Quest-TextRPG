...
void CFindGuardsTask::findFirstGuard()
{
    ...
    Console::printLn(tr("But unfortunately, he and the other guards scattered and lost each other. Most "
...
void CFindGuardsTask::rescueGuardFromMafia()
{
    ...
    Console::printLn(tr(
        "The bookkeeper dude looks at his abacus and tells you the amount: \"Including "
    ...
    {
        Console::printLn(tr("A lot of money for such a mess of a guy. But... you need his help."));
    ...
    if (menu.execute() == pay)
    {
        Console::printLn(tr(
            ...
            _mafiaMoney,
            CC::ccReset()));
        Console::printLn(tr("The messed up guard looks relieved as the mafia dudes set him free. you grab him and leave."));
...
void CFindGuardsTask::fightBossMonster()
{
    ...
    Console::printLn(tr("{} has been caught by a gigantic {}. The guard hangs crossways in the mouth of the "
                        "huge, man-eating plant. The plant chews on him, only struggeling with his armor.",
                        nameOfCurrentGuard(),
                        boss.name()));
    Console::printLn(tr(
        "Your memory seems to be awesomne. you still remember all of the guards names. Also, the plant "
        ...
        boss.name()));
    Console::printLn(tr("But seriously, is that really \"Flea-eater\"?"));
    ...
    {
        Console::printLn(tr("Probably it is not. And you don't want to risk a fight right now. Guards Armors are sturdy, "
                            "even for such a big monster. This leaves you the time for more preparation."));
        Console::confirmToContinue();
        return;
    }

    Console::printLn(tr("You approch the giant plant. {}\"Flea-eater? Is that you?\", you ask hopefully, "
                        "excited to see your old friend again.",
                        CC::bgLightGreen(),
                        CC::ccReset()));
    Console::printLn(tr("It is not!"));
    Console::printLn(tr("The plant drops its food, and attacks you."));
    CBattle battle(&boss);
    battle.fight();

    Console::printLn(tr("This has not been your old friend. You remember that he did not grow very large, after you set "
                        "him free. Also, you might have forgotten to give him water. And stepped on him several times. So "
                        "no, this was surely not him."));
    Console::printLn(tr("At least, you managed to free another guard."));
    CGameManagement::getPlayerInstance()->addSupportCompanion(new CGuardCompanion(nameOfCurrentGuard()));
...
void CFindGuardsTask::collectStuff()
{
    ...
    if (_stuffCollectionStarted == false)
    {
        Console::printLn(tr(
            ...
            nameOfCurrentGuard()));
        Console::printLn(tr("As with the others, {} is willing to help you, but he can't without his stuff, "
        ...
        Console::br();
        Console::printLn(tr("What items he did throw away, you ask?"));
        listMissingParts();
    }
    else
    {
        Console::printLn(tr("You find {} where you left him. Relaxed, sitting under a tree, and waiting for "
                            "you, bringing him his stuff.",
                            nameOfCurrentGuard()));
        Console::br();
        if (CGuardStuff::hasAll())
        {
            Console::printLn(tr("And this time, you have everything. {} is excited to have his precious "
        ...
        {
            Console::printLn(tr(
                "But still, there are things missing. {} still needs some items to be ready to follow you.",
                nameOfCurrentGuard()));
            listMissingParts();
        }
    }
...
std::string CFindGuardsTask::nameOfCurrentGuard() const
{
    ...
    case 4:
        ...
        break;
    default:
        return tr("By the way, who is this guy???");
    }
    return std::string();
}
...
void CFindGuardsTask::fightCannibalHorde()
{
    ...
    Console::printLn(tr("It is already smelling like bacon, so you should hurry, if you want to rescue this guard alive."));
    ...
    if (menu.execute() == retreatAction)
    {
        ...
        Console::printLn(tr(
            "There are many cannibals. And you are alone. {} is a big guy, it will take a while "
    ...
    Console::printLn(tr("Of course, if genocides are not the foundation for a good and healthy relationship "
                        "between hero and princess, what else could be?"));

    Console::printLn(tr("You untie {} and lead him to the others.", nameOfCurrentGuard()));
    CGameManagement::getPlayerInstance()->addSupportCompanion(new CGuardCompanion(nameOfCurrentGuard()));
