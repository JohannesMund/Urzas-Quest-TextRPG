...
{
    Console::printLn(tr("After following the river for too long, after drinking salty water one too many times..."));
    Console::printLn(tr(
    ...
    Console::br();
    Console::printLn(tr("Go closer and have a look?"));

    if (CMenu::executeYesNoMenu() == CMenu::no())
    {
        Console::printLn(tr("This is too weird, even for your standards."));
        return;
    }

    Console::printLn(tr(
        "\"Who are you guys?\" you ask. \"{}\", replys the mermaid. \"{}\", the merboyman-thing.",
        Ressources::Game::bimmelchen(),
        Ressources::Game::pimmelchen()));
    Console::printLn(tr(
        "\"And what are you doing here?\", you ask, and realize, that those two are starting to get on "
    ...
    Console::printLn(tr("\"Do you want a reason to cry?\" You do not wait for an answer, and attack."));
    Console::br();
    Console::confirmToContinue();
