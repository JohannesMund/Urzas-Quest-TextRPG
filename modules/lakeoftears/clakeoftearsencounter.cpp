...
void CLakeOfTearsEncounter::execute(const std::string_view& moduleName)
{
    ...
    Console::printLn(tr("During your journey, you pass a tiny villag. Even though, you passed this region several times, "
                     "you never took notice of this particular village. Maybe because it is just too small."));
    Console::printLn(tr("The villagers look poor, sick and weak. A very old, shaky man asks you for some water. You feel "
                     "pittry for and give him your bottle."));
    Console::printLn(tr(
        "\"Back than, before the war, {0} would never have let that happen, good times!\" "
    ...
    Console::br();
    Console::printLn(tr("Do you want to offer some help?"));

    if (CMenu::executeAcceptRejectMenu() == CMenu::accept())
    {
        Console::br();
        Console::printLn(tr("You remember what it means, to be a hero, agree to help the poor people."));
        Console::printLn(tr("The root of all their misery seems to be the salty river, so you should go up the stream and "
    ...
    Console::printLn(tr("Salty water is a \"them-problem\" not a \"you-problem\" so you decide to leave them and continue "
